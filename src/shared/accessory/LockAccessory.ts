...
export default class LockAccessory extends BaseAccessory {
  ...
  configureLockTargetState() {
    ...
    this.mainService().getCharacteristic(this.Characteristic.LockTargetState)
      ...
      .onSet(async value => {
        const result = await this.deviceManager.getLockTemporaryKey(this.device.id);
        if (!result || !result.ticket_id) {
          return;
        }
        await this.deviceManager.sendLockCommands(this.device.id, result.ticket_id, (value === UNSECURED));
      });
  }
