...
import { createDelegate, sanitizeName } from '../util/util';
...
export default class AccessoryFactory {
  static readonly platformAccessoryOverrides:Partial<
      {
        [K in keyof PlatformAccessory<TuyaPluginAccessoryContext>]: (original: PlatformAccessory<TuyaPluginAccessoryContext>[K]) =>
          PlatformAccessory<TuyaPluginAccessoryContext>[K]
      }
    > = {
      addService: ((original: any) => (...args: any[]) => {
        if (1 < args.length) {
          const arg1 = args[0];
          const arg2 = args[1];
          const left = args.slice(2);
          return original(arg1, sanitizeName(arg2), ...left);
        } else {
  ...
  static createAccessory(
    ...
    platformAccessory: PlatformAccessory<TuyaPluginAccessoryContext>,
    device: TuyaDevice,
  ): BaseAccessory {
    const accessory = createDelegate(platformAccessory, AccessoryFactory.platformAccessoryOverrides);

    let handler: BaseAccessory | undefined;
    platform.log.debug(`device:${device.id}, category:${device.category}, accessory:${accessory.displayName}`);
    ...
    if (handler && !handler.checkRequirements()) {
      platform.log.warn('checkRequirements');
      handler = undefined;
    }
  ...
  static resolveAccessoryByProductID(
    platform: TuyaPlatform,
  ...
  static resolveAccessoryByCategory(
    platform: TuyaPlatform,
    accessory: PlatformAccessory<TuyaPluginAccessoryContext>,
    category: string,
  ): BaseAccessory | undefined {
  ...
  static configAccessory(platform: TuyaPlatform, accessory: PlatformAccessory) {
    ...
    const configs = platform.platformConfig.cloud?.serviceInformationOverrides;
    if (!configs) {
      return;
    }
    ...
    configs.filter(config => config.device_id === sn).forEach(config => {
      try {
        ...
        if (config.firmwareRevision) {
          ...
          if (before !== config.firmwareRevision) {
        ...
        platform.log.error(`index out of bound.:${config.index}, error:%o`, _e);
      }
    });
  }
}